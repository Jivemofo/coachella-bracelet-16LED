...
uint32_t Wheel(byte WheelPos) {
  ...
  void theaterChaseUpdate() {
        for (int i=0; i < strip.numPixels(); i=i+2) {
          strip.setPixelColor(i+count1, Wheel(i+count2++));    //turn every third pixel on
        }
        strip.show();
    ...
    if (currentBrightness== lastBrightness %120){currentBrightness--;}
    else if (currentBrightness ==lastBrightness %50) {currentBrightness++;}
